...
테마 스탬프 최종 도안은 `assets/stamps/themes/`의 SVG 파일로 관리한다. 파일명과 `viewBox`를 유지하면 앱 코드를 수정하지 않고 디자인만 교체할 수 있다. AI Hub 문양 유형·상징을 참고해 재제작한 공용 배경은 `assets/stamps/patterns/`에 포함되며, 모든 실행 환경에서 동일한 SVG를 사용한다.

...
`start_server.ps1`이 `.env`를 읽고 `/api/weather`, `/api/weather/mid-land`, `/api/weather/mid-temperature`에서 기상청 API를 대리 호출하므로 인증키가 브라우저에 노출되지 않음. Encoding/Decoding 인증키 모두 사용할 수 있으며, 키를 바꾼 뒤에는 서버를 다시 시작해야 함.

앱은 먼저 사용자가 고른 출발지의 위도·경도를 기상청 격자 `nx/ny`로 변환하고 단기예보의 12시·18시 `TMP`, `SKY`, `PTY`, `POP`을 조회함. 단기예보에 없는 여행 날짜는 광주·전남 중기육상예보와 광주 중기기온예보로 자동 보충해 오전·오후 날씨, 강수확률, 최저·최고기온을 표시함. 단기예보는 강수형태(`PTY`)가 감지되면, 중기예보는 날씨 문구에 비·눈·소나기가 포함되면(강수확률 수치는 판단에 사용하지 않음) 해당 Day만 우천 조건으로 판단하고, 그 Day는 실내(`indoor`) 장소만 추천함. 두 예보 범위 밖 날짜, 인증키 누락·미등록, 통신 오류가 발생하면 일정 추천은 계속하고 날씨만 미반영 상태로 안내.
...
앱은 사용자가 고른 출발지의 위도·경도를 기상청 격자 `nx/ny`로 변환하고, 현재 시점의 최신 발표본에서 선택 날짜의 정확한 12시·18시 `TMP`, `SKY`, `PTY`, `POP`을 조회. 결과 화면에는 기온, 강수확률, 강수형태를 표시함. 강수형태 감지 또는 날씨 문구의 비·눈·소나기로 우천 조건을 판단하고, 그 Day는 실내(`indoor`) 장소만 추천함. 예보 범위 밖 날짜, 인증키 누락·미등록, 통신 오류가 발생하면 일정 추천은 계속하고 날씨만 미반영 상태로 안내.

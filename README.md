...
테마 스탬프 최종 도안은 `assets/stamps/themes/`의 SVG 파일로 관리한다. 파일명과 `viewBox`를 유지하면 앱 코드를 수정하지 않고 디자인만 교체할 수 있다. AI Hub 문양 유형·상징을 참고해 재제작한 기본 배경은 `assets/stamps/patterns/`에 포함되며, 로컬 AI Hub 선별 자산이 있으면 해당 문양이 우선 적용된다.

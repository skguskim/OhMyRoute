...
앱은 사용자가 고른 출발지의 위도·경도를 기상청 격자 `nx/ny`로 변환하고, 현재 시점의 최신 발표본에서 선택 날짜의 정확한 12시·18시 `TMP`, `SKY`, `PTY`, `POP`을 조회. 결과 화면에는 기온, 강수확률, 강수형태를 표시함. 비·눈 또는 강수확률 60% 이상이면 우천 조건으로 판단하고 `rainOk` 장소 중 실내 관광지에 추가 우선순위를 부여함. 예보 범위 밖 날짜, 인증키 누락·미등록, 통신 오류가 발생하면 일정 추천은 계속하고 날씨만 미반영 상태로 안내.

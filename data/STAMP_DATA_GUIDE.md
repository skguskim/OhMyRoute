...
- 관광지는 장소별 디자인을 만들지 않고 데이터 카테고리에 대응하는 고정 테마 디자인을 사용하며,
  AI Hub 문양 유형과 상징은 테마별 공용 배경을 설계하는 참고 근거로 사용합니다.
- 앱이 직접 사용하는 편집 가능한 최종 도안은 `assets/stamps/themes/*.svg`에 보관합니다.
  이 SVG는 직접 제작한 자산이며 AI Hub 원본·가공 이미지를 포함하지 않습니다.
- 다른 PC에서도 같은 문양 배경이 보이도록 AI Hub의 유형·상징 정보를 참고해 재제작한 반복형 SVG를
  `assets/stamps/patterns/*.svg`에 제공합니다. 앱은 로컬 원본 대신 모든 환경에서 이 공용 SVG를 사용합니다.
...
이 선별 목록은 문양 유형 분석과 공용 SVG 디자인 검수에만 사용합니다. 실행 중인 앱은
`data/local-stamps/`를 요청하지 않습니다.
...
5. 테마마다 최종 모티프 하나를 확정한 뒤 직접 재제작한 SVG만 서비스 자산으로 내보냅니다.
